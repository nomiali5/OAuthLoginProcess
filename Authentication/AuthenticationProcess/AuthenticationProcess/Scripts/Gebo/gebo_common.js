    ...
    gebo_sidebar = {
        init: function() {
			...
			if($(window).width() > 979){
                if(!$('body').hasClass('sidebar_hidden')) {
                    if( $.cookie('gebo_sidebar') == "hidden") {
                        $('body').addClass('sidebar_hidden');
                        $('.sidebar_switch').toggleClass('on_switch off_switch').attr('title', 'Show Filters');
                    }
                } else {
                    $('.sidebar_switch').toggleClass('on_switch off_switch').attr('title', 'Show Filters');
                }
            ...
            $('.sidebar_switch').click(function(){
                ...
                if( $('body').hasClass('sidebar_hidden') ) {
                    ...
                    $('.sidebar_switch').attr('title', "Hide Filters");
                } else {
                    $.cookie('gebo_sidebar', 'hidden');
                    $('body').addClass('sidebar_hidden');
                    $('.sidebar_switch').addClass('off_switch');
                    $('.sidebar_switch').attr('title', "Show Filters");
                }
				gebo_sidebar.info_box();
				gebo_sidebar.update_scroll();
				$(window).resize();
        ...
        update_scroll: function() {
			if($('.antiScroll').length) {
				if( $(window).width() > 979 ){
					$('.antiscroll-inner,.antiscroll-content').height($(window).height() - 80);
				} else {
